...
module "azure-resource-group" {
  ...
  rg_location = var.rg.rg.location.US
  rg_name     = var.rg.rg.name
}
...
module "azure_public_ip" {
  ...
  rg_location                = module.azure-resource-group.rg_location
  rg_name                    = module.azure-resource-group.rg_name
  public_ip_name             = var.public_ip.ip.name
...
module "azure-network-interface" {
  ...
  rg_name                                 = module.azure-resource-group.rg_name
  rg_location                             = module.azure-resource-group.rg_location
  ip_configuration_subnet_id              = module.azure_subnet.azurerm_subnet_id
  ip_configuration_public_ip_address_id   = module.azure_public_ip.azurerm_public_ip_id
  azurerm_network_interface_name          = var.nic.nic.name
  azurerm_network_interface_ip_configuration_name = var.nic.nic.ip_config_name
...
module "azure-connect-nwtwork-securitygroup" {
  ...
  connect_nsg_id        = module.azure-securitygroup.azurerm_securitygroup_id
  connect_nsg_nic_id    = module.azure-network-interface.azurerm_network_interface_id
}
...
module "azure-vm" {
  ...
  computer_name                    = var.vm.vm.computer_name
  admin_username                   = var.vm.vm.admin_username
